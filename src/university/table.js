...
function Table (props) {
    ...
    useEffect(() => {
        // Promise.all((props.professors)).then(async (values) => {
        //     setProfs(values);
        //     })
            ...
            props.courses.map((course, index) => (
                <bs.Row style={{paddingTop: "1.5rem", paddingBottom: "1.5rem", borderTop: "2px solid black"}} key={index}>
                    ...
                    <bs.Col style={{fontSize: "2.5rem", paddingTop: ".8rem"}} md="1">
                        <bs.Row className={course.userRating === VOTE_UP ? CLASS_VOTE_UP : CLASS_NO_VOTE } style={{ cursor: "pointer", paddingLeft: "3rem"}} onClick={() => handleRatingClick(course.id, VOTE_UP, updateCourseMutation, course.score)}>
                            <FontAwesomeIcon icon={faSortUp}/>
                        </bs.Row>
                        <bs.Row className={course.userRating === VOTE_DOWN ? CLASS_VOTE_DOWN : CLASS_NO_VOTE } style={{ cursor: "pointer", paddingLeft: "3rem"}} onClick={() => handleRatingClick(course.id, VOTE_DOWN, updateCourseMutation, course.score)}>
                    ...
                    <bs.Col md="4">
                        ...
                        <bs.Row style={{fontSize: "1.35rem"}}>
                            <strong>{course.code}</strong>
                        </bs.Row>
                        <bs.Row style={{fontSize: "1.2rem"}} >
                            {course.score} Points
                        </bs.Row>
                    </bs.Col>
        ...
        return(
            profs.map((professor, index) => (
                <bs.Row style={{paddingTop: "1.5rem", paddingBottom: "1.5rem", borderTop: "2px solid black"}} key={index}>
                    ...
                    <bs.Col md="3">
                        <bs.Row style={{fontSize: "1.5rem"}}>
                            <strong><Link to={`${match.url}/${URL_PARAM_PROFESSORS}/${professor.id}`}>{professor.name}</Link></strong>
                        </bs.Row>
                        <bs.Row style={{fontSize: "1.5rem"}} >
                    ...
                    <bs.Col style={{fontSize: "1.2rem"}} md="4">
                        ...
                        {/* <bs.Row>
                            <strong>College</strong> {": " + professor.department.school.name}
                        </bs.Row> */}
                    </bs.Col>
                    <bs.Col style={{fontSize: "1.8rem", paddingTop: "2rem", cursor: "pointer"}} md="1">
                        <bs.Dropdown>
