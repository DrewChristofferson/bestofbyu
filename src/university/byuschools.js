...
import { ratingsByUserAndContent, professorsByScore, coursesByScore} from '../graphql/queries';
...
function BYUSchools() {
    ...
    async function getDataCourses(nextTokenIn) {
        let apiData;
        if(nextTokenIn){
            apiData = await API.graphql({ query: coursesByScore, variables: {type: 'Course', score: {ge: 0}, sortDirection: 'DESC', limit: 490, nextToken: nextTokenIn} });
        }else {
            apiData = await API.graphql({ query: coursesByScore, variables: {type: 'Course', score: {ge: 0}, sortDirection: 'DESC', limit: 10} });
        }
    ...
    async function getDataProfessors(nextTokenIn) {
        let apiData;
        if(nextTokenIn){
            apiData = await API.graphql({ query: professorsByScore, variables: {type: 'Professor', score: {ge: 0}, sortDirection: 'DESC', limit: 490, nextToken: nextTokenIn} });
        }else {
            apiData = await API.graphql({ query: professorsByScore, variables: {type: 'Professor', score: {ge: 0}, sortDirection: 'DESC', limit: 10} });
        }
        const professorsFromAPI = apiData.data.professorsByScore.items;
    ...
    let uploadDifficulty = async() => {
        ...
        //                 } 
        //             });
        //             console.log(courses[i].code, "successfully updated")
        //             } catch (e) {
        //                 return e;
        //             }
        //     } else{
